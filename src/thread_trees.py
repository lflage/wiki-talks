...
tree = Tree()
tree.create_node("00", 0, data=Reply(thread_text))
stack = [0]
...
for ix, split in enumerate(replies_text.split("\n:")[1:],start=1):
    ...
    if depth  > len(stack):
        tree.create_node(str(depth)+str(ix), ix, parent = ix-1, data=Reply(text))

    elif depth == len(stack):
        # Something is fishy here
        tree.create_node(str(depth)+str(ix), ix, parent = ix-1, data=Reply(text))
        print('same level')
        print("depth: {}, len(stack): {}, ix: {} ".format(depth,len(stack),ix))
        print(text)

    elif depth < len(stack):
        dif = len(stack) - depth
        stack = stack[:-dif]
        tree.create_node(str(depth)+str(ix), ix, parent = stack[-1], data=Reply(text))
